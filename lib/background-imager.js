...
// array of various media query expressions for pixel ratios
var MEDIA_EXPRESSIONS_PIXEL_DENSITY = [
    "(-webkit-min-device-pixel-ratio: PIXEL_DENSITY)",
    "(min--moz-device-pixel-ratio: PIXEL_DENSITY)",
    "(-o-min-device-pixel-ratio: PIXEL_DENSITY/1)",
    "(min-device-pixel-ratio: PIXEL_DENSITY)",
    "(min-resolution: 192dpi)",
    "(min-resolution: PIXEL_DENSITYdppx)"
]
...
var BackgroundImager = {
    ...
    // FIXME: include replacement for "max" attribute
    // FIXME: include calculation for "min/max-resolution" = 1.5x as dpi
    // TODO: refactor MEDIA_EXPRESSIONS_PIXEL_DENSITY operations as method
    ...
    generateMediaQueriesWithPixelDensity: function (density, exps) {
        var queries = [];
        ...
        for (var i = 0, length = MEDIA_EXPRESSIONS_PIXEL_DENSITY.length; i < length; ++i) {
            var revisedExps = [];
            revisedExps = revisedExps.concat(exps);
            revisedExps.push(MEDIA_EXPRESSIONS_PIXEL_DENSITY[i].replace(/PIXEL_DENSITY/, density));
    ...
    generateRuleSet: function (filename, filepath, options, callback) {
        ...
        var file = path.join(filepath, filename),                   // "/fs/path/to/image1x.png"
            ...
            ruleset = {                                             // RuleSet object
                selector: "." + settings.classPrefix + ifh.getClassname(filename),
                rules: {}
            };
        ...
        im(file).size(function (err, size) {
            ...
            if (!err) {
                ...
                var ratio = ifh.getRatio(filename) || 1,
    ...
    getImageFileInfo: function (imageFilePath, callback) {
        ...
        im(imageFilePath).size(function (err, size) {
            ...
            imageFileInfo.height = size.width;
    ...
    createMediaRules: function (imageFilenames, filepath, options, callback) {
        ...
        var abstractMediaRules = [{
            // 1x
            queries: null,
            images: imageFilenames.filter(ifh.isNotSmallImage).filter(ifh.is1xImage)
        },{
            // 2x
            queries: this.generate2xMediaQueries(),
            images: imageFilenames.filter(ifh.isNotSmallImage).filter(ifh.is2xImage)
        },{
            // mobile 1x
            queries: [
            ...
            images: imageFilenames.filter(ifh.isSmallImage).filter(ifh.is1xImage)
        },{
            // mobile 2x
            queries: this.generate2xMediaQueries([
                "(max-width: 480px)"
            ]),
            images: imageFilenames.filter(ifh.isSmallImage).filter(ifh.is2xImage)
        }];
